...
function updateGameBoard(data){
	...
	var cards = data.lastPlayed;
	var cards_length = cards.length;
	var $current = jQuery(".player").eq(data.index);
	for(var m = 0; m < cards_length; m++){
		var item = cards[m];
...
$("#play-button").click(function(){
	...
	$.post('http://localhost:8888/act', x_indexes, function(u_data) {
		...
		if(data.successful !== false) //this means that the action was successful
		{
			$("#center").html("");
