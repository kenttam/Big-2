  ...
  RulesEngine = (function() {
    ...
    RulesEngine.prototype.checkIfMoveIsValid = function(cards, cardsInCenter) {
      if (cardsInCenter.length === 0) {
        return this.validHand(cards);
      } else if (this.isSingle(cardsInCenter)) {
        if (this.isSingle(cards)) {
          return this.validSinglePlay(cards, cardsInCenter);
        } else {
          return false;
        }
      } else if (this.isPair(cardsInCenter)) {
        if (this.isPair(cards)) {
          return this.validPairPlay(cards, cardsInCenter);
        } else {
          return false;
        }
      } else if (this.validFiveCardHand(cardsInCenter)) {
        if (this.validFiveCardHand(cards)) {
          return this.validFiveCardsPlay(cards, cardsInCenter);
        } else {
          return false;
        }
      }
    ...
    RulesEngine.prototype.validSinglePlay = function(cards, cardsInCenter) {
      if (this.compareSingleCard(cards[0], cardsInCenter[0]) > 0) {
        return true;
      } else {
        return false;
      }
    ...
    RulesEngine.prototype.compareSingleCard = function(a, b) {
    ...
    RulesEngine.prototype.validHand = function(cards) {
      return this.isSingle(cards) || this.isPair(cards) || this.validFiveCardHand(cards);
    };

    RulesEngine.prototype.validFiveCardHand = function(cards) {
      return cards.length === 5 && (this.isStraight(cards) || this.isFlush(cards) || this.isFullHouse(cards) || this.isFourOfAKind(cards));
    };
